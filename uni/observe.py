...
from uni.steer import Direction, read_direction
...
@dataclass(frozen=True)
class Logprob:
    """How likely the model finds the state it wrote, per token of it."""
    ...
    def read(self, step: Step) -> float:
        return self.model.reply_logprob(self.template.render(step.previous), step.state)
