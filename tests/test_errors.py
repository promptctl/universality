...
class Gone:
    """A stdout with nothing on the other end, the way `| head` leaves one once head has gone."""
...
def test_a_consumer_that_stops_reading_is_not_a_failure(capsys, tmp_path, monkeypatch):
    ...
    assert command(SWEEP) == EXIT_PIPE
    assert capsys.readouterr().err == ""  # nothing failed, so nothing is said about it
